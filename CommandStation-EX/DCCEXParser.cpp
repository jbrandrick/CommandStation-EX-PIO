...
void DCCEXParser::parse(Print *stream, byte *com, RingStream * ringStream)
{
    ...
    case 'Q': // SENSORS <Q>
        DCC_MANAGER->printAllSensors (stream);
        return;

    case 's': // <s>
        StringFormatter::send(stream, F("<p%d>\n"), DCCWaveform::mainTrack.getPowerMode() == POWERMODE::ON);
        StringFormatter::send(stream, F("<iDCC-EX V-%S / %S / %S G-%S>\n"), F(VERSION), F(ARDUINO_TYPE), DCC::getMotorShieldName(), F(GITHUB_SHA));
        DCC_MANAGER->printAllTurnouts (stream); // WAS
        DCC_MANAGER->printAllOutputs (stream);
        DCC_MANAGER->printAllSensors (stream);
  ...
  switch (paramCount)
  {
    ...
    case 0: // <Z> list Output definitions
      if (DCC_MANAGER->outputs->size () > 0) {
        DCC_MANAGER->sendDefOutputs (stream);
        return true;
      }
      return false;
  ...
  switch (paramCount)
  {
    case 0: // <T>  list turnout definitions
      if (DCC_MANAGER->turnouts->size () > 0) {
        DCC_MANAGER->sendDefTurnouts (stream);
        return true;
      }
      return false;
  ...
  switch (paramCount)
  {
    ...
    case 0: // <S> list sensor definitions
      if (DCC_MANAGER->sensors->size () > 0) {
        DCC_MANAGER->sendDefSensors (stream);
        return true;
      }
      return false;
